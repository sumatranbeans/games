// Alias Auction - Minimal Variant - Controller App
...
class AliasAuctionController {
  ...
  init() {
    ...
    if (!this.roomCode) {
      this.showError('join', 'No room code. Scan QR again.');
      return;
    }
  ...
  connectWebSocket() {
    ...
    this.ws.onclose = () => {
      this.showError('join', 'Connection lost. Refresh.');
    };
  }
  ...
  joinGame() {
    ...
    if (!name) {
      this.showError('join', 'Enter your name');
      return;
    }
  ...
  submitDescription() {
    ...
    if (!description) {
      this.showError('description', 'Enter a description');
      return;
    }
  ...
  showResults(payload) {
    ...
    resultText.className = 'result ' + (payload.success ? 'success' : 'fail');
