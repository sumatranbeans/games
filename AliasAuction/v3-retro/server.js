...
app.get('/api/qrcode/:roomCode', async (req, res) => {
  try {
    ...
    const qrDataUrl = await QRCode.toDataURL(url, {
      width: 300,
      margin: 2,
      color: { dark: '#00FF00', light: '#1A0A2E' }
    });
      ...
      switch (type) {
        ...
        case 'JOIN': {
          ...
          if (result.success) {
            ...
            ws.send(JSON.stringify({
              type: 'JOINED',
              payload: { playerId, playerName: result.player.name, color: result.player.color }
            }));
        ...
        case 'START_GAME': {
          ...
          if (currentPlayerId !== room.hostId && !isTV) {
            ws.send(JSON.stringify({
              type: 'ERROR',
              payload: { message: 'ONLY HOST CAN START' }
            }));
            return;
          }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log('');
  console.log('================================================');
  console.log('   ___   __    __  ___   ___    ___  ');
  console.log('  / _ | / /   / / / _ | / __|  / _ \\ ');
  console.log(' / __ |/ /__ / / / __ | \\__ \\ / /_\\ \\');
  console.log('/_/ |_/____//_/ /_/ |_|/___//_/   \\_\\');
  console.log('   A U C T I O N   [RETRO ARCADE]');
  console.log('================================================');
  console.log('');
  console.log(`  SERVER: http://${LOCAL_IP}:${PORT}`);
  console.log(`  TV: http://${LOCAL_IP}:${PORT}/tv`);
  console.log('');
  console.log('  INSERT COIN TO PLAY!');
  console.log('================================================');
  console.log('');
});
