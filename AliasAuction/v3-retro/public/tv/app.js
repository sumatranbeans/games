// Alias Auction - Retro Arcade Variant - TV Display App
...
class AliasAuctionTV {
  ...
  updatePlayersList(players) {
    ...
    players.forEach((player, index) => {
      ...
      row.innerHTML = `
        <span class="player-rank" style="color: ${player.color}">${index + 1}P</span>
        <span class="player-name" style="color: ${player.color}">${player.name}</span>
        <span class="player-score" style="color: ${player.color}">${player.score || 0}</span>
      `;
      this.elements.playersList.appendChild(row);
    });
  }
  ...
  updateBidDisplay() {
    ...
    document.getElementById('bidder-name').textContent = this.state.currentBidderName || 'NO BIDS';
  }
  ...
  updateScoreboard() {
    ...
    this.state.players.forEach(player => {
      ...
      item.innerHTML = `
        ...
        <span class="score-pts" style="color: ${player.color}">${player.score}</span>
      `;
  ...
  showGameOver(payload) {
    ...
    document.getElementById('winner-score').textContent = (winners[0]?.score || 0) + ' PTS';
    ...
    payload.scoreboard.forEach((player, index) => {
      ...
      row.style.borderColor = player.color;
      row.innerHTML = `
        <span class="hs-rank" style="color: ${player.color}">${index + 1}</span>
        <span class="hs-name" style="color: ${player.color}">${player.name}</span>
        <span class="hs-score" style="color: ${player.color}">${player.score}</span>
      `;
