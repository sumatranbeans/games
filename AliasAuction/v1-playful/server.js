  ...
  switch (phase) {
    ...
    case PHASES.DESCRIBING:
      ...
      startPhaseTimer(roomCode, TIMERS.DESCRIBING, () => {
        // If no description submitted, treat as failure
        if (!room.description) {
    ...
    try {
      ...
      switch (type) {
        ...
        case 'SUBMIT_DESCRIPTION': {
          ...
          if (result.success) {
            // Clear the timer early since description was submitted
            if (room.timerInterval) {
              clearInterval(room.timerInterval);
              room.timerInterval = null;
            }
        ...
        case 'VOTE': {
          ...
          if (result.success) {
            ...
            if (result.allVoted) {
              // All votes in, move to results early
              if (room.timerInterval) {
                clearInterval(room.timerInterval);
                room.timerInterval = null;
              }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('  ALIAS AUCTION - Playful Variant');
  console.log('='.repeat(50));
  console.log('');
